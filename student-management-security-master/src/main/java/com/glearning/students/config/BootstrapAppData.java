...
public class BootstrapAppData {
	...
	public void initializeUsersData(ApplicationReadyEvent readyEvent) {
			...
			User praveen = new User("praveen", passwordEncoder.encode("welcome"));
			User prabhas = new User("prabhas", passwordEncoder.encode("welcome"));
			
			Role userRole = new Role("ROLE_USER");
			Role adminRole = new Role("ROLE_ADMIN");
			
			praveen.addRole(userRole);
			
			prabhas.addRole(userRole);
			prabhas.addRole(adminRole);
			
			this.userRepository.save(praveen);
			this.userRepository.save(prabhas);
